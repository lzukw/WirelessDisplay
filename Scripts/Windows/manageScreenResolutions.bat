...
IF "%ACTION%" == "GET" (
    FOR /F "DELIMS=, TOKENS=1" %%G IN ('..\..\ThirdParty\ScreenRes\screenres.exe /V /S') DO echo %%G
)

REM Echo all available screen-resolitions of the primary screen to stdout.
IF "%ACTION%" == "ALL" (
    FOR /F "DELIMS=, TOKENS=1" %%G IN ('..\..\ThirdParty\ScreenRes\screenres.exe /V /L') DO echo %%G
)
...
REM delimiter.
IF "%ACTION%" == "SET" (
    FOR /F "tokens=1,2 delims=x" %%i IN ('echo %RESOLUTION%') DO ..\..\ThirdParty\ScreenRes\screenres.exe /V /X:%%i /Y:%%j
)
